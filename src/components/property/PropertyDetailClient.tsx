...
export default function PropertyDetailClient({
  ...
  useEffect(() => {
    if (!isSupabaseConfigured()) {
      ...
      if (sandbox) {
        ...
        const found = mockList.find((l: any) => String(l.id) === String(propertyId));
        if (found) {
          setDbProperty({
            id: String(found.id),
            title: found.title,
            ...
            agentId: found.posted_by || "agent-1",
    ...
    const fetchProperty = async () => {
      try {
        ...
        setDbProperty({
          ...
          amenities: ["Approved Asset", "Verified Listing"],
          agentId: data.posted_by || "agent-1",
  ...
  const property = dbProperty || initialProperty;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleShare}
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-blue-600 transition-colors shadow-xs relative"
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
          <div>
            <div className="flex flex-wrap gap-2 items-center mb-3">
              <span
                className={`rounded-full px-3 py-1 text-2xs font-extrabold tracking-wider uppercase ${
                  isLuxury
            ...
            <h1
              className={`text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 ${
                isLuxury ? "font-serif text-slate-950" : ""
              }`}
            >
              {property.title}
            </h1>
          ...
          <div className="text-left md:text-right shrink-0">
            <p
              className={`text-2xl sm:text-3xl font-black text-slate-900 ${
                isLuxury ? "text-amber-600 font-serif" : ""
              }`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="relative aspect-video w-full rounded-2xl bg-slate-100 overflow-hidden shadow-sm group">
              ...
              {property.images.length > 1 && (
                <>
                  <button
                    onClick={prevImage}
                    className="absolute left-4 top-1/2 -translate-y-1/2 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-slate-700 hover:text-slate-900 shadow-sm border opacity-0 group-hover:opacity-100 transition-opacity z-10"
                  ...
                  <button
                    onClick={nextImage}
                    className="absolute right-4 top-1/2 -translate-y-1/2 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-slate-700 hover:text-slate-900 shadow-sm border opacity-0 group-hover:opacity-100 transition-opacity z-10"
                ...
                {property.images.map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      activeImageIdx === idx ? "bg-white w-5" : "bg-white/60 w-2"
                    }`}
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
                {property.amenities.map((item) => (
                  <div
                    key={item}
                    className="flex items-center gap-2 text-xs text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100"
                  >
                ...
                <div className="mt-4 space-y-3.5 text-xs text-slate-600">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-slate-800 text-right">
                      Tenant responsible for electric, water & trash
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="sticky top-24 space-y-6">
              <ContactAgentForm agent={agent} propertyName={property.title} propertyId={property.id} />
